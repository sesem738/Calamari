...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    self.ego_bp = self._create_vehicle_bluepprint('custom', color='49,8,8')
    ...
    self.lidar_trans = carla.Transform(carla.Location(x=10.0, z=self.lidar_height))
    self.lidar_bp = self.world.get_blueprint_library().find('sensor.lidar.ray_cast')
    self.lidar_bp.set_attribute('channels', '64')
    self.lidar_bp.set_attribute('range', '100')
    self.lidar_bp.set_attribute('points_per_second', '250000')
    self.lidar_bp.set_attribute('rotation_frequency', '20')

    # Camera sensor
    self.camera_img = np.zeros((self.obs_size, self.obs_size, 3), dtype=np.uint8)
    self.camera_trans = carla.Transform(carla.Location(x=-0.2, z=1.7))
  ...
  def _get_obs(self):
    ...
    z_bins = [-self.lidar_height-10, -self.lidar_height+4.55, 10]
